...
import React, { useEffect, useState, forwardRef, useMemo,  ReactNode, useCallback, Suspense, useRef } from "react";
...
const JoystickComponents = (props: EcctrlJoystickProps) => {
    ...
    const joystickStickGeo = useMemo(() => new THREE.CylinderGeometry(0.3, 0.3, 2, 6), [])
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            joystickDiv.removeEventListener("dblclick", pressBtn1)
            window.visualViewport?.removeEventListener("resize", onWindowResize)
        }
    }, [onTouchMove, windowSize])
...
const ButtonComponents = ({ buttonNumber = 1, ...props }: EcctrlJoystickProps) => {
     ...
     */
    const curButton1Pressed = useJoystickControls((state: any) => state.curButton1Pressed)
    ...
    const onPointerUp = () => {
        releaseAllButtons();
        api.start({
            buttonTop1BaseScaleY: curButton1Pressed ? 0.5 : 1,
            buttonTop1BaseScaleXAndZ: curButton1Pressed ? 1.15 : 1,
            buttonTop2BaseScaleY: 1,
            buttonTop2BaseScaleXAndZ: 1,
            buttonTop3BaseScaleY: 1,
            buttonTop3BaseScaleXAndZ: 1,
...
export const EcctrlJoystick = forwardRef<HTMLDivElement, EcctrlJoystickProps>((props, ref) => {
    ...
    const buttonWrapperStyle: React.CSSProperties = {
        ...
        width: props.buttonHeightAndWidth || '120px',
        right: '1.1rem',
        bottom: '2.1rem',
    }
