...
import React, { useEffect, useState, forwardRef, useMemo,  ReactNode, useCallback, Suspense } from "react";
...
const JoystickComponents = (props: EcctrlJoystickProps) => {
    ...
    const joystickDiv: any = document.querySelector("#ecctrl-joystick")
    const pressButton1 = useJoystickControls((state:any) => state.pressButton1)
    const releaseAllButtons = useJoystickControls((state: any) => state.releaseAllButtons)
     ...
     */
    const joystickBaseGeo: any = useMemo(() => new THREE.CylinderGeometry(2.3, 2.1, 0.3, 16), [])
    const joystickStickGeo: any = useMemo(() => new THREE.CylinderGeometry(0.3, 0.3, 2, 6), [])
    const joystickHandleGeo: any = useMemo(() => new THREE.SphereGeometry(1.4, 8, 8), [])

    /**
     * Joystick component materials
     */
    const joystickBaseMaterial: any = useMemo(() => new THREE.MeshNormalMaterial({ transparent: true, opacity: 0.3 }), [])
    const joystickStickMaterial: any = useMemo(() => new THREE.MeshNormalMaterial({ transparent: true, opacity: 0.3 }), [])
    const joystickHandleMaterial: any = useMemo(() => new THREE.MeshNormalMaterial({ transparent: true, opacity: 0.7 }), [])

    /**
     * Joystick store setup
     */
    const setJoystick = useJoystickControls((state: any) => state.setJoystick)
    const resetJoystick = useJoystickControls((state: any) => state.resetJoystick)
    ...
    const pressBtn1 = ()=>{
        ...
        setTimeout(()=>releaseAllButtons(),500)
    ...
    useEffect(() => {
        ...
        // joystickDiv.addEventListener("dblclick", pressBtn1)
    ...
    return (
        <Suspense fallback="null">
            ...
            <animated.group rotation-x={springs.topRotationX} rotation-y={springs.topRotationY}>
                ...
                <mesh geometry={joystickHandleGeo} material={joystickHandleMaterial} position={[0, 0, 2.5]} {...props.joystickHandleProps} />
            </animated.group>
        </Suspense>
    )
}
...
const ButtonComponents = ({ buttonNumber = 1, ...props }: EcctrlJoystickProps) => {
     ...
     */
    const curButton2Pressed = useJoystickControls((state: any) => state.curButton2Pressed)
    const buttonLargeBaseGeo: any = useMemo(() => new THREE.CylinderGeometry(1.1, 1, 0.3, 16), [])
    const buttonSmallBaseGeo: any = useMemo(() => new THREE.CylinderGeometry(0.9, 0.8, 0.3, 16), [])
    const buttonTop1Geo: any = useMemo(() => new THREE.CylinderGeometry(0.9, 0.9, 0.5, 16), [])
    const buttonTop2Geo: any = useMemo(() => new THREE.CylinderGeometry(0.9, 0.9, 0.5, 16), [])
    const buttonTop3Geo: any = useMemo(() => new THREE.CylinderGeometry(0.7, 0.7, 0.5, 16), [])
    const buttonTop4Geo: any = useMemo(() => new THREE.CylinderGeometry(0.7, 0.7, 0.5, 16), [])
    const buttonTop5Geo: any = useMemo(() => new THREE.CylinderGeometry(0.7, 0.7, 0.5, 16), [])

    /**
     * Button component materials
     */
    const buttonBaseMaterial: any = useMemo(() => new THREE.MeshNormalMaterial({ transparent: true, opacity: 0.3 }), [])
    const buttonTop1Material: any = useMemo(() => new THREE.MeshNormalMaterial({ transparent: true, opacity: 0.5 }), [])
    const buttonTop2Material: any = useMemo(() => new THREE.MeshNormalMaterial({ transparent: true, opacity: curButton2Pressed ? 0.7: 0.5}), [curButton2Pressed])



    const buttonTop3Material: any = useMemo(() => new THREE.MeshNormalMaterial({ transparent: true, opacity: 0.5 }), [])
    const buttonTop4Material: any = useMemo(() => new THREE.MeshNormalMaterial({ transparent: true, opacity: 0.5 }), [])
    const buttonTop5Material: any = useMemo(() => new THREE.MeshNormalMaterial({ transparent: true, opacity: 0.5 }), [])

    const buttonDiv:any = document.querySelector("#ecctrl-button")
     ...
     */
    const pressButton1 = useJoystickControls((state:any) => state.pressButton1)
    const pressButton2 = useJoystickControls((state: any) => state.pressButton2)
    const pressButton3 = useJoystickControls((state: any) => state.pressButton3)
    const pressButton4 = useJoystickControls((state: any) => state.pressButton4)
    const pressButton5 = useJoystickControls((state: any) => state.pressButton5)
...
export const EcctrlJoystick = forwardRef<HTMLDivElement, EcctrlJoystickProps>((props, ref) => {
    ...
    const joystickWrapperStyle: React.CSSProperties = {
        ...
        position: 'absolute',
        zIndex: editPopup?'':'9999',
        transform: 'translate(-50%,0)',
        height: props.joystickHeightAndWidth || '120px',
        width: props.joystickHeightAndWidth || '120px',
        left: '50%',
        bottom: '34rem',
    }
    ...
    const buttonWrapperStyle: React.CSSProperties = {
        ...
        position: 'absolute',
        zIndex: editPopup?'':'9999',
        height: props.buttonHeightAndWidth || '120px',
        width: props.buttonHeightAndWidth || '125px',
        right: '1rem',
        bottom: '34rem',
    }
        ...
        <div ref={ref}>
            <div id="ecctrl-joystick" style={joystickWrapperStyle} onContextMenu={(e) => e.preventDefault()}>
                <Canvas
                    ...
                    camera={{
                        zoom: 20,
                        position: props.joystickCamPosition || [0, 0, 60],
                    }}
                >
            ...
            </div>
            {
                ...
                <div id="ecctrl-button" style={buttonWrapperStyle} onContextMenu={(e) => e.preventDefault()}>
                    <Canvas
                        ...
                        gl={{ alpha: true }}
                        camera={{
                            zoom: props.buttonCamZoom || 20,
                            position: props.buttonCamPosition || [0, 0, 60],
                        }}>
...
export type EcctrlJoystickProps = {
    ...
    buttonCamPosition?:any;
    buttonGroup1Position?: any;
    buttonGroup2Position?: any;
    buttonGroup3Position?: any;
    buttonGroup4Position?: any;
    buttonGroup5Position?: any;
